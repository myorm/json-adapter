        ...
        serialize({ ErrorTypes, MyORMAdapterError }) {
            return {
                ...
                forInsert(data) {
                    ...
                    let hasNonUniqueKeyViolation = false;
                    ...
                    config.$data[table].forEach(r => hasNonUniqueKeyViolation = uniqueKeys.filter(k => {
                        if(uniques.has(r[k])) return true;
                        uniques.add(r[k]);
                        return false;
                    }).length > 0);
                    if(hasNonUniqueKeyViolation) {
